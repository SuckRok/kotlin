 ...
 *
 * This set does not support removal operations like [remove], [removeAll], [retainAll].
 * Also [iterator] returns an iterator which does not support [MutableIterator.remove].
 */
@suppress("UNCHECKED_CAST", "CAST_NEVER_SUCCEEDS")
public class SmartHashSet<T> : AbstractSet<T>() {
    ...
    override fun add(e: T): Boolean {
        when {
            ...
            size < ARRAY_THRESHOLD - 1 -> {
                val arr = obj as Array<T>
                for (element in arr) {
                    if (element == e) return false
                }
                obj = Arrays.copyOf(arr, size + 1).apply { set(this@SmartHashSet.size, e) }
            }
            size == ARRAY_THRESHOLD - 1 -> {
                val arr = obj as Array<T>
                for (element in arr) {
                ...
                obj = LinkedHashSet(arr.asList()).apply { add(e) }
            }
            else -> {
